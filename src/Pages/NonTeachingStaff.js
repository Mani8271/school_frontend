...
const NonTeachingStaff = () => {
  ...
  const handleAddStaff = () => {
    setStaff((prev) => [
      { ...newStaff, branch: selectedBranch },
      ...prev,
    ]);
  };
  ...
  const handleFormSubmit = () => {
    ...
    if (editingStaff) {
      setStaff((prev) =>
        prev.map((s) => (s.id === editingStaff.id ? { ...editingStaff, ...newStaff, branch: selectedBranch } : s))
      );
    } else {
      handleAddStaff();
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-500 bg-opacity-75 overflow-y-auto">
          <div className="relative w-full max-w-3xl p-6 bg-white rounded-md shadow-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {/* Emergency Contact */}
              <div>
                <label htmlFor="emergencyContact" className="block font-medium text-gray-700">Emergency Contact Number</label>
                <input
                  type="text"
                  id="emergencyContact"
          ...
          {currentStaff.length > 0 ? (
            currentStaff.map((s) => (
              <tr key={s.id} className="border-b hover:bg-gray-50">
                <td className="px-4 py-3 border ">{s.id}</td>
                ...
                <td className="px-4 py-3 border">{s.mobile}</td>
